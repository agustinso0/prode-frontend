# Tournament Contracts Specification

## Domain: tournament-contracts

### Purpose

Define frontend-only contracts, fixtures, and query behavior for public tournament and match previews while no backend exists.

### Requirement: Public DTO Schemas

The frontend MUST define Zod DTO schemas for public tournament and public match data. DTOs MUST include stable identifiers, public names, status, kickoff timestamps, team labels, tournament references, and display-safe metadata. Schemas MUST parse unknown data before UI use and MUST avoid backend endpoint assumptions.

#### Scenario: Valid mock DTOs parse

- GIVEN World Cup 2026-style public tournament and match DTOs
- WHEN the schemas parse them
- THEN parsing succeeds with typed DTO output

#### Scenario: Invalid contract data fails fast

- GIVEN a DTO has an invalid status, missing identifier, or invalid kickoff timestamp
- WHEN schema parsing runs
- THEN validation fails before UI rendering

### Requirement: Builders, View Models, and Query Hooks

The feature MUST provide realistic World Cup 2026-style builders and feature-owned TanStack Query hooks backed by mock data. Hooks MUST expose validated public data, stable query keys, loading/success/error states, and UI-ready view models without global mocks or direct component fetch calls.

#### Scenario: Hooks provide mock public data

- GIVEN the backend does not exist
- WHEN landing query hooks run
- THEN they resolve active/upcoming tournaments and upcoming matches from validated builders

#### Scenario: View models fit landing needs

- GIVEN parsed DTOs are transformed for landing display
- WHEN components consume them
- THEN UI receives formatted labels and grouped preview data without raw unparsed DTOs

### Requirement: Strict TDD Verification

Implementation MUST start with failing tests and use `npx pnpm test` as the required runner. Tests MUST cover schemas, builders, query hooks, loading/empty/error/success page states, accessibility-critical labels, and absence of emojis in public copy.

#### Scenario: Quality gate protects the slice

- GIVEN the implementation is ready for handoff
- WHEN `npx pnpm test` runs
- THEN all slice tests pass without a running backend
